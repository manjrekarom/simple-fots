...
def crop_area(im, polys, tags, crop_background = False, max_tries = 50):
    '''
    Crops input image. The crop image will have a subset of text boxes, but 
    none of those text boxes will be partially cropped.
    ...
    # below code crops the image but makes sure that a subset of text
    # boxes selected are not partially cropped.
    # For e.g. if there are 8 text boxes, it is possible to select 3 of them
    # but neither of those 3 should be cropped.
    # select x and y values which don't contain a text box
    h_axis = np.where(h_array == 0)[0]
    w_axis = np.where(w_array == 0)[0]
    if len(h_axis) == 0 or len(w_axis) == 0:
        return im, polys, tags, np.array(len(polys))
    for i in range(max_tries):
        # select xmin, xmax, ymin and ymax for cropping the image
        # select two points on x axis
        xx = np.random.choice(w_axis, size = 2)
        # pad xmin on left
        xmin = np.min(xx) - pad_w
        # BUG: Should be padded on right
        # pad xmax on right; previously left
        # Change: changed sign from - to +
        xmax = np.max(xx) + pad_w
        # make sure x lies in the range of 0 to w
        xmin = np.clip(xmin, 0, w - 1)
        xmax = np.clip(xmax, 0, w - 1)
        # select two points on y axis
        yy = np.random.choice(h_axis, size = 2)
        ymin = np.min(yy) - pad_h
        # BUG: Should be padded at bottom
        # Change: changed sign from - to +
        ymax = np.max(yy) + pad_h
        # make sure y lies in the range of 0 to h
        ...
        if xmax - xmin < 0.1 * w or ymax - ymin < 0.1 * h:
            ...
            continue
        if polys.shape[0] != 0:
            # selects text boxes that completely lie in the selected x and y ranges
